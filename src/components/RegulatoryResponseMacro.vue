...
const macroRegistry = {
  '-ft-en': firstTouch,
  '-ft-zh': zhFirstTouch,
  '-ndp-ra': acknowledgement,
  '-ndp-te-R': reporter,
  '-ndp-te-A': accused,
  '-ndp-mapping': ndpFinalMapping,
  '-od-refund-later': refund,
  '-od-mapping': openDoors,
  '-ap-ra-G': accessibilityGuestRa,
  '-ap-ra-H': accessibilityHostRa,
  '-ap-mapping': apFinalMapping,
  '-rrr-ft': reviewFirstTouch,
  '-rrr-outcome-mapping': reviewOutcome,
}
...
const filteredMacros = computed(() => {
  const match = macro.value.match(/-[\w-]*$/)
  if (!match) return macros
  return macros.filter(m => m.startsWith(match[0]))
})
